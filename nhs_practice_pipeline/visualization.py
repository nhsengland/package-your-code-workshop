...
class GraphingStage(PipelineStage):
    ...
    def run(self, context):
        ...
        if "monthly_by_status" in summary_stats:
            ...
            ax1.set_title(
                "Monthly Appointments by Status", fontsize=14, fontweight="bold"
            )
            ax1.set_xlabel("Month")
            ax1.set_ylabel("Number of Appointments")
            ax1.legend(
                title="Appointment Status", bbox_to_anchor=(1.05, 1), loc="upper left"
            )
        ...
        if "hcp_type_summary" in summary_stats:
            ...
            sns.barplot(data=hcp_data, x="total_appointments", y="hcp_type", ax=ax2)
            ax2.set_title(
        ...
        if "mode_by_month" in summary_stats:
            ...
            ax3.set_title(
                "Appointment Mode Trends Across Months", fontsize=14, fontweight="bold"
            )
            ax3.set_xlabel("Month")
            ax3.set_ylabel("Number of Appointments")
            ax3.legend(
                title="Appointment Mode", bbox_to_anchor=(1.05, 1), loc="upper left"
            )
            plt.xticks(rotation=45)
            plt.tight_layout()
        ...
        if "regional_summary" in summary_stats:
            ...
            sns.barplot(
                data=regional_data, x="total_appointments", y="region_name", ax=ax4
            )
            ax4.set_title(
